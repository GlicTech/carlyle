...
export default class ProTimeLogEntry extends NavigationMixin(LightningElement) {
  ...
  loadData() {
    ...
    const normalize = (rows, mode) => {
      const normalized = (rows || []).map((r) => {
        const row = {
          ...r,
          entries: r.entries ? r.entries.map((e) => ({ ...e })) : []
        };
        row.entries.forEach((entry) => {
          ...
          entry.weekendClass = this.weekendMap[entry.entryDate]
            ? "weekend-cell"
            : "";
    ...
    Promise.all([
      ...
      .then(([onRes, offRes]) => {
        ...
        const calcTotals = (rows) => {
          ...
          return this.dateHeaders.map((h, i) => ({
            key: h.key,
            value: totals[i]
          }));
        };
  ...
  computeTotals() {
    ...
    this.grandCombinedTotal = this.combinedTotals.reduce(
      (a, b) => a + b.value,
      0
    );
  }
  ...
  handleValueChange(event) {
    ...
    const otherVal =
      this.editedEntries[`${consultantId}-${date}-${otherMode}`] || 0;

    if (newVal + otherVal > 1) {
      this.showToast(
        "The total on-site and off-site time cannot exceed a day.",
        `Please adjust your entries ${otherVal}`,
        "error"
      );
      event.target.value = "";
      return;
      ...
      for (const entry of row.entries) {
        for (const mode of modes) {
          ...
          if (value !== undefined) {
            ...
            if (!modeMap.get(mode).has(row.consultantId))
              modeMap.get(mode).set(row.consultantId, []);
