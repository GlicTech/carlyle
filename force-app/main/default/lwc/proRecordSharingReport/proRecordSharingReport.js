...
export default class ProRecordSharingReport extends NavigationMixin(LightningElement) {
    @track groupedList = [];
    @track searchKey = '';
    isLoading = true;
        ...
        return this.groupedList
            .map(group => ({
                ...
                records: group.records.filter(
                    r =>
                        (r.RecordName && r.RecordName.toLowerCase().includes(this.searchKey)) ||
                        (r.UserOrGroupName && r.UserOrGroupName.toLowerCase().includes(this.searchKey))
                )
            }))
