...
export default class ProRecordSharing extends LightningElement {
    ...
    wiredShares(value) {
        ...
        if (data) {
            ...
            this.existingShares = [];
            this.showToast('Error loading shares', (error.body && error.body.message) || 'Unknown error', 'error');
        }
    }
    ...
    handleSelect(event) {
        ...
        if (!this.selectedUsers.find(u => u.Id === recordId)) {
            this.selectedUsers = [
                ...this.selectedUsers,
                { Id: recordId, Name: recordName, iconName: 'standard:user', Type: 'User' }
            ];
        }
        this.searchKey = '';
        this.results = [];
    ...
    handleAccessChange(event) { this.accessLevel = event.detail.value; }
    handleCancel() { this.dispatchEvent(new CloseActionScreenEvent()); }

    handleSave() {
        if (!this.selectedUsers.length) {
            this.showToast('Error', 'Please select at least one External Rep.', 'error');
            return;
        }
        ...
        .catch(error => {
            this.showToast('Error sharing record', (error.body && error.body.message) || 'Unknown error', 'error');
        });
    }
    ...
    revokeRecordShare({ recordId: this.recordId, userOrGroupId })
        .then(() => {
            this.existingShares = this.existingShares.filter(s => s.Id !== userOrGroupId);
            this.showToast('Access revoked', 'Sharing removed successfully.', 'success');
            return refreshApex(this.wiredSharesResult); 
        })
        .catch(error => {
            this.showToast('Error revoking access',
